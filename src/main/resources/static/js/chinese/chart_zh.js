...
function showHeader(){
    ...
    for(let i=0; i<geochartArray.length-1; i++) {
        if(geochartArray[i]['국가명']!=='中华人民共和国'){
    ...
    let htmlCountry= (geochartArray.length-1) +" 个国家";
...
function showTable(index){  // 페이징처리
    ...
    if(page==1){    // 현재 페이지가 첫페이지이면
        pagehtml += '<li class="page-item disabled"><button class="page-link" type="button" onclick="showTable('+(page-1)+')">前</button> </li>';
    }else{  // 현재페이지가 첫페이지가 아니면
        pagehtml += '<li class="page-item"><button class="page-link" type="button" onclick="showTable('+(page-1)+')">前</button> </li>';
    }
    ////////////////////////////////////////////////////////////////////////////////
    ...
    if(page==totalpage || totalpage == 0){
        pagehtml += '<li class="page-item disabled"><button class="page-link" type="button" onclick="showTable('+(page)+')">下一个</button> </li>';
    }else{
        pagehtml += '<li class="page-item"><button class="page-link" type="button" onclick="showTable('+(page+1)+')">下一个</button> </li>';
    }
...
function drawVisualization() {
    ...
    for(let i=0; i<geochartArray.length; i++){
        data.addRows([[{v:geochartArray[i]['ISO'],f:geochartArray[i]['국가명']},
            geochartArray[i]['확진자'],'确诊病例 : '+geochartArray[i]['확진자']+'\n' +
            '类似疾病病例 : '+geochartArray[i]['유증상자']+'']]);
        // ivalue[geochartArray[i]['ISO']] = 'http://www.google.com';
        if(i===geochartArray.length-1){
            totalconfirmed = geochartArray[i]['확진자총합'];
            let code='<span>当今全球确诊病例数 : '+totalconfirmed+'</span>';
